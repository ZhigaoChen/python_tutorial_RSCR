...
MODELPATH = 'model' + MODEL + '/bnb.model'
...
def load_separate_sing_read_dataset_to_tain_test(dataRead, dataSing):
    x_train, y_train = load_saved_dataset(dataRead)
    x_test, y_test = load_saved_dataset(dataSing)
    return x_train, x_test, y_train, y_test
...
def grid_search(x_train, y_train):
    print 'grid search...'
    # 用信息增益启发式算法建立决策树
    pipeline = Pipeline([('clf', BernoulliNB())])  # not
