        ...
        for filename in filenames:
            if '.DS_Store' not in filename and '.doc' not in filename:
                ...
                for segment in segments:
                    count += 1
                    vad_audio = vad_audio + segment
                    # print segment
                out_dir = os.path.dirname(file_path.replace(data, data + 'vad'))
                if os.path.isdir(out_dir):
                    pass
                else:
                    os.makedirs(out_dir)
                path = out_dir + '/' + os.path.basename(file_path)
                write_wave(path, vad_audio, sample_rate)
