...
MODELPATH = 'model' + MODEL + '/gbc.model'
...
def grid_search(x_train, y_train):
    print 'grid search...'
    # 用信息增益启发式算法建立决策树
    pipeline = Pipeline([('clf', GradientBoostingClassifier())])  # not
...
def basic_model(x_train, x_test, y_train, y_test):
    if not os.path.isfile(MODELPATH):
        clf = GradientBoostingClassifier(random_state=1007, n_estimators=100, learning_rate=0.1, verbose=1,
                                         max_depth=1, )
